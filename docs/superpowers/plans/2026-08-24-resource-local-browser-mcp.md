...
**Status:** Historical implementation plan. Its direct Browser authorization and normal-profile Windows attachment were superseded by the Local Tool Broker and the [dedicated MCP Chrome design](2026-08-24-browser-fast-windows-agent-browser.md). The rollout evidence below is retained as archaeology, not current setup guidance.
...
**Historical architecture:** Expose one `browser` MCP facade in the personal 1MCP composition. The facade published the Chrome DevTools MCP tool catalog once, defaulted calls to the normal native Windows Chrome profile, and routed `browser_target=linux` calls to a managed WSLg Chrome child. Both resource-local children stayed internal to the facade and the outward provider used the single 1MCP authorization tag `browser`. The current Local broker and dedicated-profile architecture is defined by the superseding plan above.
