...
For the personal Browser domain, the outer 1MCP exposes only Local `tool_list`, `tool_schema`, and `tool_call` under `tag:browser`; the Local provider starts a private inner 1MCP over stdio in normal direct mode, and that inner config contains the Browser facade. The renderer writes the inner config before the outer config so a hot reload cannot start Local against a missing file. Stock lazy `tool_invoke` is intentionally not in the Browser path because direct mode preserves top-level rich MCP results.

Chrome remote debugging must be enabled in the normal Windows Chrome profile before the Browser facade can dispatch Windows-targeted calls. Internally that child uses `--autoConnect` plus `--user-data-dir=%LOCALAPPDATA%\\Google\\Chrome\\User Data`; this was selected after bare `--autoConnect` timed out in qualification. Do not expose a public debugging port. Browser tools default to this Windows target; `browser_target=linux` selects a separate managed Chrome profile through WSLg.
