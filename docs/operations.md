...
For the personal Local domain, the outer 1MCP exposes only Local `tool_list`, `tool_schema`, and `tool_call` under `tag:local`; the Local provider starts a private inner 1MCP over stdio in normal direct mode, and that inner config currently contains the Browser facade. The renderer writes the inner config before the outer config so a hot reload cannot start Local against a missing file. Stock lazy `tool_invoke` is intentionally not in the Browser path because direct mode preserves top-level rich MCP results.
