...
Task 7 was still being implemented when this audit reached the Terminal checkpoint. Its MCP server/renderer was not yet available for exact result-size measurement. The reviewed frozen Task-7 design is nevertheless explicit enough to decide the format trigger: `terminal_read` returns raw unread/snapshot text, `terminal_list` returns concise native session summaries, administrative tools return concise native text, and every successful tool returns `TextContent` without `structuredContent`. Exact implementation confirmation therefore remains pending Task 7, but the frozen result contract does not create a codec-eligible payload. If the implementation later violates that contract by exposing a large repeated record payload, this trigger audit should be reopened against that exact payload rather than against a synthetic Terminal schema.
...
| `terminal_list` / status summary | concise per-session native lines containing live/dead state, exact dead exit status, dimensions, attachment and human-control state | **exact MCP size pending Task 7 implementation** | potentially one short repeated line per session | frozen design says yes; no evidence of a large session population or costly result | encoding a small status list would reduce human/model scanability | named facts matter, but labels in concise text are sufficient; no consumer requires JSON fields | design forbids `structuredContent`, so a codec would replace already-concise text or create duplication | exact status/exit values matter | no actual retained payload demonstrates substantial context cost; reopen only if finished Task 7 produces one | no current trigger; implementation confirmation pending |
| Terminal `open` / `send` / `resize` / `close` results | concise administrative acknowledgements/errors; `send` must not echo input | **exact MCP sizes pending Task 7 implementation** | none/materially bounded | frozen design requires concise native text | conversion adds ceremony and risks echoing sensitive input if poorly designed | no | structured copy would duplicate tiny acknowledgements | action identity/error code matters | negative/immaterial | no |
...
- Terminal wait/resume remains deferred pending real Task-7 evidence and therefore contributes no retained payload class to this audit.
...
- `terminal_list` is the only new design that could superficially resemble a record list, but there is not yet an actual production payload showing substantial context cost, and the frozen contract intentionally renders a concise native summary. A hypothetical large JSON session table is not evidence and is not model-facing.
