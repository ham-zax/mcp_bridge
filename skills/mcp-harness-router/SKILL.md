...
- Private logical MCP capability -> Local. If the logical server/tool is already selected, skip catalog discovery, load its exact `tool_schema` only when the schema is not already known, then invoke `tool_call(server=..., tool=..., arguments=...)`. If the action is unknown, use selective `tool_list(server=..., query=...)` first. Reuse loaded schemas. When Agent Browser applies, preserve its selected `browser-fast` vs `browser-devtools` route and `browser_target`; never rewrite the browser backend here.
