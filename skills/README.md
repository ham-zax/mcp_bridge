...
- `agent-browser` is materialized from the installed ChatGPT browser Skill with one harness-specific routing change: resource-local Windows/WSLg Chrome work goes through the single Browser MCP facade with a locality selector, while isolated/Electron automation keeps the existing `agent-browser` CLI workflow.
