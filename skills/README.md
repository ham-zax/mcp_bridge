...
- `agent-browser` is materialized from the installed ChatGPT browser Skill with harness routing for two Local browser surfaces: routine Windows/WSLg interaction uses experimental `browser-fast` `observe`/`execute`, including bounded read-only site/platform/policy memory returned by observation; DevTools diagnostics use `browser`, and isolated/Electron automation keeps the existing `agent-browser` CLI workflow.
- `job-application` is the ChatGPT-side component of the removable private `job-application` extension. It resolves canonical job-search sources from the enabled extension state at runtime, uses `agent-browser`/Local for external ATS interaction, uploads only the namespaced `job-application.resume.current` artifact, keeps logged-in LinkedIn manual-only, and stages reusable site mechanics through the Dev-only browser-memory candidate workflow instead of embedding personal facts or machine-specific repository paths in the Skill.
- `x-content` is the ChatGPT-side content-intelligence workflow for technical X posts, Quotes, threads, replies, critique, variants, and evidence-based learning. It keeps hooks/voice/topic knowledge out of Browser memory, can read an optional private `x-content` workspace from enabled extension state, and remains useful without the extension or any X browser mutation path.
