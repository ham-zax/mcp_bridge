...
- `agent-browser` is materialized from the installed ChatGPT browser Skill with harness routing for two Local browser surfaces: routine Windows/WSLg interaction uses experimental `browser-fast` `observe`/`execute`, DevTools diagnostics use `browser`, and isolated/Electron automation keeps the existing `agent-browser` CLI workflow.
